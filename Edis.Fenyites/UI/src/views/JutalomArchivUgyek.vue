<template>
  <div>
    <div class="page-header py-10 px-15 d-flex align-items-center">
      ...
      >
        Fogvatartottak archív jutalmi ügyei
      </span>
    </div>
...
export default {
  ...
  methods: {
    ...mapActions({}),
    OnIntezetValasztas() {
      this.GetArchivJutalmiUgyek(this.ugyEve);
    },
    GetUgyszam(f) {
      return getUgyszam(f);
    },
    ...
    async GetArchivJutalmiUgyek(ev) {
      try {
        let result = await apiService.GetJutalmiUgyekArchiv({ ev: ev });
        this.jutalmiUgyek = Object.freeze(result);
      } catch (error) {
        console.log(error);
  ...
  computed: {
    ...
    SzurtJutalomUgyek: function() {
      ...
      var filteredList = [];
      var fegyelmiUgyekTmp = this.jutalmiUgyek.slice();
    ...
    szurok() {
      let szurtFegyelmiUgyek = this.SzurtJutalomUgyek;
