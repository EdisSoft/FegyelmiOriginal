...
export default {
  name: 'helyszini-szemle-naplobejegyzes',
  data() {
    return {
      toolTipOptions: {
  ...
  methods: {
    ...
    NaploBejegyzesSzerkesztes() {
      ...
      eventBus.$emit('Modal:helyszini-szemle', {
        state: true,
        data,
      });
    },
  ...
  computed: {
    ...
    fejlec() {
      ...
      if (
        this.naplobejegyzes.TipusCimkeId == Cimkek.NaploTipus.HelysziniSzemle
      ) {
        ...
        }</em> helyszíni szemlét tartott <em>${this.$options.filters.toShortDateKeltezes(
          this.naplobejegyzes.JegyzokonyvLezarasDatuma
        )}</em>.`;
      }
      return 'Helyszíni szemle';
    },
  },
  watch: {},
  components: {},
