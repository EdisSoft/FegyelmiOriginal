  ...
  <div class="panel" v-if="fegyelmiUgy && naplobejegyzes">
    <div class="panel-heading" :id="panelId" role="tab">
      <a
        ...
        v-b-toggle="panelId"
      >
        <span v-if="!naplobejegyzes.Vegleges"
          >Feddés fenyítés kezdeményezve.
        </span>
        <span v-else>
          <span v-if="!naplobejegyzes.IsFogvatartottElfogadta">
            Feddés fenyítés kiszabva, de a fogvatartott nem fogadta el.
          </span>
          <span v-else>
            Feddés fenyítés kiszabva.
          </span>
        </span>
      </a>
    </div>
...
export default {
  ...
  methods: {
    ...
    NaploBejegyzesSzerkesztes() {
      ...
      eventBus.$emit('Modal:reintegracios-tiszt-dontese-feddes', {
        state: true,
        data,
      });
    },
