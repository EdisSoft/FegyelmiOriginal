      ...
      >
        ...
        <div class="text-right">
          ...
          >
            ...
            I. fokú fegyelmi tárgyalás időpontját kitűzöm
          </button>
        </div>
      </div>
    </div>
...
export default {
  name: 'I-foku-fegyelmi-targyalas-elokeszitese',
  data() {
    return {
      isFormLoading: false,
      fegyelmiUgyId: 0,
  ...
  methods: {
    ...
    LoadFegyelmiUgyData: async function(fegyelmiUgyId) {
      ...
      try {
        var result = await apiService.GetElsoFokuTargyalasElokeszitese({
          id: fegyelmiUgyId,
        });
    ...
    TargyalasiIdopontKituzese: async function() {
      ...
      try {
        ...
        this.isLoading = true;
        var result = await apiService.TargyalasiIdopontKituzese({
          data: this.formData.values,
        });

        NotificationFunctions.SuccessAlert(
          'I. fokú tárgyalás kitűzve',
          result.message
        );
        ...
        NotificationFunctions.AjaxError({
          title: 'Hiba',
          text: 'Hiba az I. fokú tárgyalás kitűzésekor',
          errorObj: err,
        });
        console.log(err);
      }
