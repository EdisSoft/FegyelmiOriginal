  ...
  <basic-loader :isLoading="isFormLoading">
    <div class="modal-primary">
      <div class="modal-header bg-blue-grey-400">
        ...
        <h4 class="modal-title">
          24. Magánelzárás ideiglenesen ellenjavallt
          <p class="mt-10 mb-0 font-size-12">
            Egészségügyi és pszichológiai szempontból a magánelzárás
            ideiglenesen ellenjavallt.
          </p>
        </h4>
      </div>
            ...
            <div class="col-md-12">
              ...
              >
                ...
                <span class="text-help float-right"
                  >Végrehajthatóság, vagy felülvizsgálat dátuma</span
                >
              </k-vuelidate-error-extractor>
            </div>
          </div>
    ...
    <div class="modal-footer">
      ...
      <button
        ...
        @click="MaganelzarasEllenjavallata()"
...
export default {
  name: 'maganelzaras-ideiglenesen-ellenjavallt',
  data() {
    return {
      isFormLoading: false,
      title: 'Magánelzárás ideiglenesen ellenjavallt',
      fegyelmiUgyIds: [],
      naplobejegyzesIds: [],
      mentesLoading: false,
      isMounted: false,
  ...
  methods: {
    ...
    LoadFormData: async function(fegyelmiUgyIds, naplobejegyzesIds) {
      ...
      try {
        var result = await apiService.GetMaganelzarasEllenjavallataModalData({
    ...
    SaveFormData: async function() {
      try {
        var result = await apiService.SaveMaganelzarasEllenjavallataModalData({
          data: {
    ...
    async MaganelzarasEllenjavallata() {
      if (!this.IsFormValid()) {
        return;
      }
  ...
  computed: {
    ...
    datePickerOptions() {
      ...
      return {
        format: 'YYYY.MM.DD',
        defaultDate: this.formData.values.Hatarido,
        useCurrent: false,
